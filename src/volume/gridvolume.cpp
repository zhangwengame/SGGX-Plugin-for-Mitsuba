...
public:
	...
	size_t getVolumeSize() const {
		...
		switch (m_volumeType) {
			...
			case ESGGX: return 1 * nEntries * m_channels;
			default:
	...
	void loadFromFile(const fs::path &filename) {
		...
		switch (type) {
			...
			case ESGGX:
				format = "sggx";
				if (m_channels != 6)
					Log(EError, "Encountered an unsupported sggx data "
					"volume data file (%i channels, only 6 are supported)",
					m_channels);
				break;
			default:
		...
		switch (m_volumeType) {
			case ESGGX: {
				for (int k = 0; k<8; ++k) {
					...
					Float factor = ((k & 1) ? fx : _fx) * ((k & 2) ? fy : _fy)
						* ((k & 4) ? fz : _fz);
					sigma = Vector(m_densityMap[m_data[index * 6]], m_densityMap[m_data[index * 6 + 1]], m_densityMap[m_data[index * 6 + 2]]);
					r = Vector(m_densityMap[m_data[index * 6+3]], m_densityMap[m_data[index * 6 + 4]], m_densityMap[m_data[index * 6 + 5]]);
					r = r * 2.0;
					r.x = r.x - 1.0; r.y = r.y - 1.0; r.z = r.z - 1.0;
					s[k][0] = sigma.x*sigma.x;
					s[k][1] = sigma.y*sigma.y;
					s[k][2] = sigma.z*sigma.z;
					s[k][3] = r.x*sigma.x*sigma.y;
					s[k][4] = r.y*sigma.x*sigma.z;
					s[k][5] = r.z*sigma.y*sigma.z;
					for (int i = 0; i < 6; i++)
						S_[i] = S_[i] + factor*s[k][i];
				}
			}
				break;
			default:
